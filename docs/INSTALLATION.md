...
> **PyCodeKG developers:** Use `poetry install --with dev` for the full dev environment. Add `-E viz` or `-E viz3d` for visualizer extras. The `extras` mechanism above is for *consumers* of the package; dev tooling lives in a Poetry group and is not published as an extra.
...
poetry install --with dev --all-extras   # everything, including dev
```
