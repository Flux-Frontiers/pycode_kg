...
| Field | Value |
|---|---|
| **CodeKG version** | 0.8.0 |
| **Branch** | `develop` |
| **HEAD commit** | `9b6218c` — fix: pass snapshot_mgr to CodeKGAnalyzer in analyze_repo() |
...
| **Repo indexed** | `/Users/egs/repos/code_kg` |
| **SQLite DB** | `.codekg/graph.sqlite` |
...
- **Q2** — `CodeKG.pack` (0.568), `_compute_span` (0.478): correct (span/lineno logic)
...
- Q2 ("missing_lineno_policy cap_or_skip fallback") → top semantic results: `CodeKG.__exit__`, `_docstring_signal`, `QueryResult.print_summary` at 0.907. **Completely unrelated.**
...
- Q1 ("snapshot freshness comparison") in legacy mode → `MainWindow.on_pick` and `CodeKG.__exit__` surface at the top.
...
It *would* be appropriate if CodeKG embedded the actual source code text (the raw function bodies as strings). In that case, CodeBERT's token-level representations would give it a structural advantage for identifier matching. But CodeKG embeds **node metadata** — qualified names, docstrings, signatures, and structural context. That's closer to natural language than Python syntax, and CodeBERT has no meaningful discrimination there.

**Bottom line:** CodeBERT produces a degenerate retrieval embedding space for CodeKG's metadata-based indexing. Do not use it.

---

### 3. `all-mpnet-base-v2` underperforms its size

768-dim, 2.4s build, yet hybrid scores trail BGE-small on every query. Q3 results in hybrid mode are particularly poor: `_tab_snippets` and `CodeKGVisitor._add_edge` surface at the top for "how does the graph get built from source code" — a UI tab widget and an edge-insertion helper. Higher cost, lower quality than BGE-small.
...
**`BAAI/bge-small-en-v1.5` is the canonical embedding model for CodeKG.**
...
The `DEFAULT_MODEL` constant in `src/code_kg/codekg.py` is set correctly. No change needed. This benchmark result locks it in.
