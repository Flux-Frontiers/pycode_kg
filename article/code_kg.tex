...
vector index (\texttt{SemanticIndex}), and an orchestrator (\texttt{CodeKG})---plus a
...
\texttt{CodeKGExtractor} is the Python-AST-backed implementation used by
\texttt{CodeKG}.  Any other knowledge graph domain (\emph{e.g.}, TypeScript,
...
\texttt{CodeKG} is a concrete \texttt{KGModule} subclass.  It provides the
Python-AST-specific extraction layer via \texttt{CodeKGExtractor} and delegates
...
are lazy. \texttt{CodeKG} supports the context manager protocol for clean resource
...
\texttt{.codekg/snapshots/} and indexed in a manifest.  The \texttt{diff\_snapshots()}
...
pre-commit git hook installed by \texttt{codekg install-hooks}.
...
Both phases are idempotent. \texttt{codekg build} wipes existing data by default;
pass \texttt{--no-wipe} to perform an upsert instead.  The lower-level
\texttt{build-sqlite} and \texttt{build-lancedb} subcommands default to upsert and
accept \texttt{--wipe} to clear first.
...
invoke \texttt{codekg} subcommands directly.  It is included for integrations that
...
The server is started via the \texttt{codekg-mcp} entry point and communicates
over \texttt{stdio} or \texttt{sse}.

\section{CLI Entry Points}

All commands are available as \texttt{codekg \textit{subcommand}} via the main
dispatcher or as standalone \texttt{codekg-\textit{name}} entry points.  Both
...
Command & Purpose \\
\midrule
\texttt{codekg build} & AST extraction $\to$ SQLite + LanceDB (wipes by default; \texttt{--no-wipe} to preserve) \\
\texttt{codekg build-sqlite} & AST extraction $\to$ SQLite only \\
\texttt{codekg build-lancedb} & SQLite $\to$ LanceDB vector index \\
\texttt{codekg update} & Incremental update (add/remove changed files) \\
\texttt{codekg query} & Hybrid query (JSON output) \\
\texttt{codekg pack} & Hybrid query + snippet pack (Markdown or JSON) \\
\texttt{codekg analyze} & Full architectural analysis (Markdown report) \\
\texttt{codekg centrality} & Structural Importance Ranking (SIR) \\
\texttt{codekg architecture} & Architecture summary \\
\texttt{codekg snapshot save|list|show|diff} & Temporal snapshot management \\
\texttt{codekg viz} & Launch the Streamlit graph explorer \\
\texttt{codekg viz3d} & Launch the 3-D PyVista knowledge-graph visualizer \\
\texttt{codekg viz-timeline} & Interactive temporal metrics visualization \\
\texttt{codekg install-hooks} & Install pre-commit git hook for auto-snapshots \\
...
\texttt{SemanticIndex}, \texttt{KGModule}, \texttt{CodeKG}---cleanly separates
