...
- Entry points and configuration: the CLI ``main()`` resolves repo, SQLite,
    and LanceDB paths with sensible defaults under ``.codekg/``.
...
``.codekg/graph.sqlite`` and ``.codekg/lancedb`` relative to ``--repo``.
...
def _get_kg() -> CodeKG:
    ...
    if _kg is None:
        raise RuntimeError(
            "CodeKG not initialised.  Run the server via 'codekg-mcp --repo /path/to/repo'"
        )
    return _kg
...
def _get_snapshot_mgr() -> SnapshotManager:
    ...
    if _snapshot_mgr is None:
        raise RuntimeError(
            "SnapshotManager not initialised.  Run the server via 'codekg-mcp --repo /path/to/repo'"
        )
...
mcp = FastMCP(
    "codekg",
    instructions=(
        "CodeKG is a hybrid semantic + structural knowledge graph for a Python codebase. "
        ...
        "paths parameter to restrict results to a specific subtree (e.g. 'src/code_kg'). "
        ...
        "'cls:src/code_kg/store.py:GraphStore'). Returns Markdown with kind, location, ID, "
...
def query_codebase(
    ...
    **rerank_mode** controls result ordering after graph expansion:
    ...
    :param paths: Comma-separated module path prefixes to include, e.g.
                  ``"src/code_kg"`` to exclude tests and scripts.
...
def get_node(node_id: str, include_edges: bool = False) -> str:
    ...
    e.g. ``cls:src/code_kg/store.py:GraphStore`` or
    ``fn:src/code_kg/store.py:GraphStore.expand``.  Use
    ...
    Example workflow::
        ...
        get_node("cls:src/code_kg/store.py:GraphStore", include_edges=True)

        # 3. Read the source
        pack_snippets("GraphStore implementation")

    :param node_id: Stable node identifier in ``<kind>:<module_path>:<qualname>``
                    format, e.g. ``fn:src/code_kg/store.py:GraphStore.expand``.
...
def graph_stats() -> str:
    ...
    """
    stats = _get_kg().stats()
    out: list[str] = ["## CodeKG Graph Statistics\n"]
...
def list_nodes(module_path: str = "", kind: str = "") -> str:
    ...
    :param module_path: Module path prefix filter (e.g. "src/code_kg/store.py").
...
def centrality(
    ...
    """
    try:
        from code_kg.analysis.centrality import (  # pylint: disable=import-outside-toplevel
...
def bridge_centrality(
    ...
    **Connectivity score** = (unique modules called + unique modules calling this) / 30 + frequency / 50
    ...
    """
    try:
        from code_kg.analysis.bridge import (  # pylint: disable=import-outside-toplevel
...
def framework_nodes(top: int = 20) -> str:
    ...
    try:
        ...
        from code_kg.analysis.framework_detector import (  # pylint: disable=import-outside-toplevel
...
def analyze_repo() -> str:
    ...
    Executes the nine-phase CodeKG analysis pipeline — complexity hotspots,
    ...
    This is the same analysis produced by ``codekg analyze``; calling it from
    ...
    analyzer = CodeKGAnalyzer(_get_kg(), console=silent, snapshot_mgr=_snapshot_mgr)
...
def explain(node_id: str, limit: int = 10) -> str:
    ...
    Given a node ID (e.g., ``fn:src/code_kg/store.py:GraphStore.expand``),
...
def rank_nodes(
    ...
    """
    from code_kg.ranking.coderank import (  # pylint: disable=import-outside-toplevel
        build_code_graph,
        compute_coderank,
...
def _parse_args(argv: list | None = None) -> argparse.Namespace:
    """
    Parse command-line arguments for the CodeKG MCP server entry point.
...
def main(argv: list | None = None) -> None:
    ...
    Initialises the CodeKG instance, the SnapshotManager, and starts the
    ...
    _snapshot_mgr = SnapshotManager(repo / ".codekg" / "snapshots", db_path=db)
