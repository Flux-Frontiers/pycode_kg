...
Click subcommands for managing temporal snapshots of CodeKG metrics:
...
from code_kg.cli.options import sqlite_option
...
@cli.group("snapshot")
def snapshot() -> None:
    """Manage temporal snapshots of CodeKG metrics."""
    pass
...
@click.option(
    ...
    default=None,
    type=click.Path(),
    help="Snapshots directory (default: .codekg/snapshots).",
)
@click.option(
    "--branch",
    default=None,
...
def save_snapshot(
    ...
    """
    Capture current CodeKG metrics and save as a temporal snapshot.
    ...
    Snapshots are stored in .codekg/snapshots/{tree_hash}.json, with a
    manifest.json tracking all snapshots and their metrics.

    Example:
        codekg snapshot save 0.5.1 --repo .
    """
    repo_root = Path(repo).resolve()
    db_path = Path(sqlite)
    snapshots_path = (
        Path(snapshots_dir).resolve() if snapshots_dir else (repo_root / ".codekg" / "snapshots")
    )
    ...
    # Load CodeKG and get docstring coverage + complexity metrics
    kg = CodeKG(
        repo_root=repo_root,
        db_path=db_path,
    ...
    try:
        analyzer = CodeKGAnalyzer(kg, snapshot_mgr=snap_mgr)
...
def list_snapshots(snapshots_dir: str | None, limit: int | None, output_json: bool) -> None:
    ...
    """
    snapshots_path = (
        Path(snapshots_dir).resolve() if snapshots_dir else (Path.cwd() / ".codekg" / "snapshots")
    )
...
def diff_snapshots(key_a: str, key_b: str, snapshots_dir: str | None, output_json: bool) -> None:
    ...
    Example:
        codekg snapshot diff 660e4f0a 3487ed5b
    """
    snapshots_path = (
        Path(snapshots_dir).resolve() if snapshots_dir else (Path.cwd() / ".codekg" / "snapshots")
    )
