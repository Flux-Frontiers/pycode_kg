...
Click subcommands for launching CodeKG visualizers:
...
_VIZ_EXTRA = 'pip install "code-kg[viz]"'
...
def viz(db: str, port: str, no_browser: bool) -> None:
    """Launch the CodeKG Streamlit visualizer."""
    ...
    click.echo(f"Launching CodeKG Explorer on http://localhost:{port}")
...
def viz3d(db: str, layout: str, width: int, height: int) -> None:
    """Launch the CodeKG 3-D PyVista knowledge-graph visualizer."""
    db_path = Path(db)
    ...
    from code_kg.viz3d import launch  # pylint: disable=import-outside-toplevel
...
def viz_timeline(snapshots: str, type: str) -> None:
    ...
    if not snapshots_path.exists():
        raise click.UsageError(
            ...
            "Run 'codekg snapshot save' first to capture snapshots."
        )
    ...
    from code_kg.viz3d_timeline import (  # pylint: disable=import-outside-toplevel
