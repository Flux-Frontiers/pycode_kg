...
- Entry point and configuration defaults: resolves ``repo_root`` and defaults
    ``db_path``/``lancedb_path`` to ``.codekg/graph.sqlite`` and
    ``.codekg/lancedb``.
...
class CodeKGAnalyzer:
    ...
    def _get_report_metadata(self, elapsed_seconds: float = 0.0) -> str:
        ...
        Collects the generation timestamp, CodeKG package version, current Git
        ...
        version = "unknown"
        try:
            from code_kg import (  # noqa: PLC0415  # pylint: disable=import-outside-toplevel
                __version__ as _v,
            )
...
This report provides a comprehensive architectural analysis of the **{repo_name}** repository using CodeKG's knowledge graph. The analysis covers complexity hotspots, module coupling, key call chains, and code quality signals to guide refactoring and architecture decisions.
...
"""
        ...
        if self.centrality_modules:
            report += (
                ...
                "Node-level detail: `codekg centrality --top 25`\n\n"
            )
...
"""
        ...
        if self.snapshot_history:
            ...
            report += "No snapshots found. Run `codekg snapshot save <version>` to capture one.\n"
...
*Report generated by CodeKG Thorough Analysis Tool — analysis completed in {elapsed_str}*
"""
    ...
    def to_markdown(self) -> str:
        ...
        out.append("# CodeKG Repository Analysis\n")
        ...
        if self.snapshot_history:
            ...
            out.append("No snapshots found. Run `codekg snapshot save <version>` to capture one.\n")
        out.append("")
...
def main(
    ...
    if not db.exists():
        console.print(
            ...
            "Run [bold]codekg build --repo .[/bold] first, then re-run analyze."
        )
        return

    try:
        from code_kg import CodeKG  # pylint: disable=import-outside-toplevel
        ...
        snapshots_dir = root / ".codekg" / "snapshots"
        ...
        from code_kg.config import (  # pylint: disable=import-outside-toplevel
            load_exclude_dirs,
            load_include_dirs,
        )
