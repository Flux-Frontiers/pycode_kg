...
def _load_kg(repo_root: str, db_path: str, lancedb_dir: str, model: str):
    ...
    configuration boundary between Streamlit controls and runtime CodeKG state.
...
def _render_sidebar() -> dict:
    ...
    """
    st.sidebar.title("CodeKG Explorer")
    st.sidebar.markdown("---")
    ...
    if build_graph_btn or build_all_btn:
        with st.sidebar:
            with st.spinner("Building graph (AST → SQLite)…"):
                try:
                    from code_kg import (  # pylint: disable=import-outside-toplevel
                        CodeKG,
                    )
