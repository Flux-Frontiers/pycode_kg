...
def _local_model_path(model_name: str) -> Path:
    ...
    Defaults to ``.codekg/models/<model>`` under the current working directory
    so the cache lives alongside the rest of the CodeKG artefacts.
    Override via the ``CODEKG_MODEL_DIR`` environment variable.
    ...
    default = str(Path.cwd() / ".codekg" / "models")
    cache_root = Path(os.environ.get("CODEKG_MODEL_DIR", default))
...
def _build_index_text(n: dict) -> str:
    ...
    **Format version 2** — includes a KEYWORDS section of de-duplicated word
    ...
    change (``codekg build-lancedb --wipe``).
